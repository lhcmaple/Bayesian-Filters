...
zPredSigmaPts=zeros(wNoise,nsp);
for k=1:nsp
    xPredSigmaPts(:,k) = feval(ffun,xSigmaPts(1:states,k),U,dt)+xSigmaPts(states+1:states+vNoise,k);
    zPredSigmaPts(:,k) = feval(hfun,xPredSigmaPts(:,k),U,dt)+xSigmaPts(states+vNoise+1:states+noises,k);
end
